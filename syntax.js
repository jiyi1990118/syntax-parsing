...
(function (syntaxFn) {
    ...
    var strGate = {
        ...
        isidentifierStart: function (cp) {
            return (cp === 0x24) || (cp === 0x5F) ||
                ...
                (cp >= 0x61 && cp <= 0x7A) ||
                (cp === 0x5C) ||
                ((cp >= 0x80) && Regex.NonAsciiidentifierStart.test(strGate.fromCodePoint(cp)));
        },
        // 变量字符
        isidentifierPart: function (cp) {
            return (cp === 0x24) || (cp === 0x5F) ||
                ...
                (cp >= 0x30 && cp <= 0x39) ||
                (cp === 0x5C) ||
                ((cp >= 0x80) && Regex.NonAsciiidentifierPart.test(strGate.fromCodePoint(cp)));
        },
        // 数字字符
    ...
    syntaxParser.prototype.expressionLex = function (atom, isAdopt) {
        ...
        atom = atom || this.nexAtom || this.atomLex();
                ...
                switch (atom.type) {
                    case atomType.Punctuator:
                        switch (atom.identity) {
                            ...
                            case "bracketsRight":
                                //检查是否能匹配语法块结束符号
                                if (this.expBlockEnd.length && this.expBlockEnd.pop() === atom.value) break;
                            default:
                                this.throwErr('语法错误,多出符号"' + atom.value + '" '+expTemp.valueType);
                                break;
                        }
                        break;
                    default:
    ...
    (function (expression) {
        ...
        expression.expTernary = function (atom, expTemp) {
            ...
            //继续检查后续表达式并连接
            this.expressionLex();
            this.expStruct = this.expConcat(struct, this.expStruct);
    ...
    syntaxParser.prototype.throwErr = function (msg,atom) {
        this.errMsg = msg;
        atom=atom||this.atoms[this.atoms.length-1];
